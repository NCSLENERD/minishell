...
int			split_piece(char *content, t_piece **fields, char **acc,
				int *started);
...
pid_t		launch_command(t_command *cmd, t_shell *shell, int prev_fd,
				int *fds);
...
int			write_heredoc_line(int fd, char *line, t_redirect *redir,
				t_shell *shell);
